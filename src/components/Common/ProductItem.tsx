...
import { getImageSrc, getServerImageSrc } from "@/helper/imageLink.helper";
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        <Image
          src={getServerImageSrc(item.thumbnail)}
          alt=""
          width={250}
          height={250}
        />
      ...
      <div className="flex items-center gap-2.5 mb-2">
        {/* <p className="text-custom-sm">({item.star_number ?? 0})</p> */}
        
        <div className="flex items-center gap-1">
          {[...Array(Number(item.star_number) || 0)].map((_, i) => (
            <Image
              key={i}
      ...
      >
        <Link href={`/shop-details/${item._id}`}> {item.product_name} </Link>
      </h3>
