...
const NewArrival = () => {
  ...
  useEffect(() => {
    const fetchLatestProducts = async () => {
      try {
        const res = await fetch(`${apiUrl}/api/v1/products/latest`, {
          next: { revalidate: 60 }, // optional caching for Next.js App Router
        });

        const json = await res.json();

        console.log("Latest products fetched:", json.data);
        // console.log("Shop Data:", shopData);

        if (res.ok) {
          setProducts(json.data || []);
        } else {
          console.error("Error fetching latest products:", json.message);
        }
      } catch (error) {
        console.error("Fetch latest products failed:", error);
      } finally {
        setLoading(false);
      }
    };
